'''
This file is modifiled
Add: MAPPERs, build_mapper
Change: build_model (can build_mapper if cfg.type in ['Mapper'])
'''
import warnings
...
#MAPPERS = Registry('mappers', parent=MMCV_MODELS)
...
def build_mapper(cfg, train_cfg=None, test_cfg=None):
    if train_cfg is not None or test_cfg is not None:
        warnings.warn(
            'train_cfg and test_cfg is deprecated, '
            'please specify them in model', UserWarning)

    return MAPPERS.build(cfg, default_args=dict(train_cfg=train_cfg,
                                                test_cfg=test_cfg))
...
def build_model(cfg, train_cfg=None, test_cfg=None):
    ...
    # elif cfg.type in ['Mapper', 'VecHDMapNet', 'PolyHDMapNet', 'DGHdmapNet']:
    #     return build_mapper(cfg, train_cfg=train_cfg, test_cfg=test_cfg)
    # else:
